...
CAGY discovers the public installed-app OAuth client from the locally installed agy binary, so normal account login, token refresh, and automatic quota failover do not depend on shell variables or checked-in credentials. Native desktop OAuth clients cannot keep these identifiers secret; the refresh tokens are tied to that client. `CAGY_GOOGLE_CLIENT_ID` and `CAGY_GOOGLE_CLIENT_SECRET` remain optional overrides for a compatible client rotation or controlled testing. Imported accounts can still be listed and inspected without opening OAuth.
...
If the shell, terminal host, Codex tool runner, or `cagy ask` process disappears, agy can continue in the visible pane. The next `cagy doctor` reconciles the journal with the verified live developer and exact JSONL transcript. It reports whether the task is still running, blocked, completed without acknowledgment, or uncertain. Safe recovery failure codes are stored for quota stop timeouts, unavailable account pools, and original-developer restore failures. A verified live working developer still wins, but an idle developer is not reported as running only because an old transcript background marker is stale. cagy never resends that task automatically.
